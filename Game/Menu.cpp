...
{
    // Jeœli to pierwszy element, ustaw go jako domyœlnie wybrany
...
void Menu::SelectNext() {
    ...
    SelectedIndex = (SelectedIndex + 1) % Items.size(); // PrzejdŸ dalej i zapêtl
...
void Menu::SelectPrev() {
    ...
    Items[SelectedIndex].Selected = false;
    SelectedIndex--;
    if (SelectedIndex < 0) SelectedIndex = Items.size() - 1; // Zapêtl od ty³u
    Items[SelectedIndex].Selected = true;
}
...
void Menu::Draw(SpriteRenderer& renderer, TextRenderer& text, glm::mat4 projection)
{
    for (const auto& item : Items)
    {
        ...
        // Obliczamy œrodek przycisku dla tekstu (prosta matematyka)
        float textX = item.Position.x + (item.Size.x * 0.1f); // ma³y margines od lewej
        float textY = item.Position.y + (item.Size.y * 0.2f); // ma³y margines od góry
